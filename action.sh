...
patch_apk() {
	...
	local cmd="java -jar $rv_cli_jar patch -b $rv_patches_jar -m $rv_integ_apk -o $patched_apk --rip-lib x86_64 --rip-lib x86 --keystore=revanced.keystore $patcher_args $stock_input"
	pr "$cmd"
	if [ "${DRYRUN:-}" = true ]; then
